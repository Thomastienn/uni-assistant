...
class LinearTransformation:
    ...
    def get_transform_mat(self):
        A_mn = Matrix([[] for _ in range(self.RM)])
    ...
    def get_transform_ADB(self, B: list[Matrix], D: list[Matrix]):
        A_db = Matrix([[] for _ in range(len(D))])
        for b_basis in B:
    ...
    def get_inverse_transform_func(self):
        ...
        def new_func(vector):
            new_a = [[0] for _ in range(len(vector))]
            for i in range(len(inverse_ta)):
                for j in range(len(inverse_ta[0])):
                    new_a[i][-1] += (inverse_ta[i][j]*vector.vR(j))
                    print(new_a)
                    print(inverse_ta[i][j], vector.vR(j))
            return Matrix(new_a)
    ...
    def inv_transform(self, vector):
        if len(vector) != self.RM or not vector.is_vector():
            assert False, "Invalid vector"
